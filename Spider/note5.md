...
- ItenPipeline
    ...
    - pipeline需要处理process_item函数
        - process_item：
            - spide提取出来的item作为参数传入，同时传入的还有spider
            - 此方法必须实现
    ...
    - Spider
        ...
        - start requests：生成Requests对象交给Scprapy下载并返回response
        - parse：根据返回的response解析出相应的item，item自动进入pipeline：如果需要解析出url，url自动交给
        requests模块，一直循环下去
        - start request：此方法仅能被调用一次，读取start urls内容并启动循环过程
        - name：设置爬虫名称
        - start_urls：设置开始第一批爬取的url
        - allow domains：spider 允许爬取的域名列表
        - stastart_request(self)：只被调用一次
        - parse
        - log：日志记录
    - 中间件（DowloaderMiddlewares）
        - 中间件是出于引擎和下载器中间的一层组件
        - 可以有很多个，被按顺序加载执行
        - 作用是对发出的请求和返回的结果进行预处理
        - 在middlewares文件中
        - 需要在settings中设置以便生效
        - 编写中间件十分简单，中间件必须是scrapy.contrib.dowliadermiddleware.DownloaderMiddleware的子类
        - 一般一个中间件完成一项功能
        - 必须实现以下一个或者多个方法
